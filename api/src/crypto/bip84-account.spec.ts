...
describe('bip84', () => {
  ...
  test('bip84', async () => {
    const account1 = Bip84Utils.fromMnemonic(exchangeMnemonic, Network.testnet, 'vpub');
    await extractTransactionsFromAccount(account1, bcService);
  });

  test('find all txs in test wallet', async () => {
    const account0 = Bip84Utils.fromMnemonic(exchangeMnemonic, Network.testnet, 'vpub');
