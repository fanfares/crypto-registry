...
export interface TestModuleOptions {
  singleNode?: boolean;
  useRealBitcoinServices?: boolean;
}
...
export const createTestModule = async (
  ...
  const singleNode = options?.singleNode ?? false;
  const useRealBitcoinServices = options?.useRealBitcoinServices ?? false;
  ...
  const apiConfigService = {
    ...
    isTestMode: true,
    bitcoinApi:  useRealBitcoinServices ? 'electrum-x' : 'mock',
