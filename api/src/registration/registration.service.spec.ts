...
describe('registration-service', () => {
  ...
  test('registration workflow', async () => {
    ...
    const message = await node2.senderService.sendBroadcastMessage(MessageType.textMessage, 'Hello World');
    const receivedMessage = await node1.dbService.messages.findOne({ id: message.id });
